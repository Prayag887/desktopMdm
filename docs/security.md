...
- Blue screen mode never invokes crash APIs, disables OS recovery/input, makes itself unkillable, or saves/relaunches prank state. Escape is not a dismissal shortcut. An explicit Exit button, normal close, QR dismissal, reboot and a five-minute timeout remain available. The health service never starts a simulation.
