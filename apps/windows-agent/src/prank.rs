...
fn page(title: &str, description: &str, action: &str) -> String {
    format!(
        r#"<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title><style>body{{margin:0;background:#081a35;color:#edf5ff;font:17px/1.6 system-ui,sans-serif;min-height:100vh;display:grid;place-items:center}}main{{width:min(440px,calc(100% - 64px));padding:32px 0}}small{{color:#8fbaff;letter-spacing:.15em}}h1{{font-size:36px;line-height:1.15}}p{{color:#c4d2e6}}button{{font:inherit;font-weight:650;border:0;border-radius:12px;background:#8fc6ff;color:#071a34;padding:16px 20px;width:100%;cursor:pointer}}button:focus-visible{{outline:3px solid white;outline-offset:5px}}footer{{margin-top:32px;font-size:14px;color:#95aac6}}</style><main><small>EMI DEVICE · BLUE SCREEN MODE</small><h1>{title}</h1><p>{description}</p>{action}<footer>Temporary, one-time link. The PC\u0027s Exit button or a reboot also ends the simulation.</footer></main></html>"#
    )
}
